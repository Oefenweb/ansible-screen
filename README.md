...
* `screen_screenrc_destinations`: [default: `{skell: dest: /etc/skel, current: dest: "{{ ansible_facts['env']['HOME'] }}"}`]: Destinations to copy the screenrc file to
...
* `screen_startup_message`: [default: `false`]: Whether to show the copyright notice during startup
* `screen_autodetach`: [default: `true`]: Whether to automatically detach the session on SIGHUP
* `screen_vbell`: [default: `false`]: Whether to use visual bell
